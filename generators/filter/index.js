...
module.exports = nodesGenerator.extend({
	...
	constructor: function() {
		...
		this.option('skip-module', {
			desc: 'Skips registerring the filter as a standalone angular module',
			type: Boolean
		});
	...
	prompting: {
		...
		promptForModuleType: function() {
			...
			var prompt = {
				type: 'list',
				name: 'type',
				message: chalk.green('What type of filter is this?'),
				choices: [
					'common',
					'models',
					'modules'
		...
		promptForModuleLocation: function() {
			...
			var prompt = {
				type: 'input',
				name: 'location',
				message: chalk.green('Which folder should i create the filter in?'),
				default: this.name
			};
