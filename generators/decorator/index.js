...
module.exports = nodesGenerator.extend({
	...
	constructor: function() {
		...
		this.option('skip-module', {
			desc: 'Skips registerring the decorator as a standalone angular module',
			type: Boolean
		});
	...
	prompting: {
		...
		promptForModuleType: function() {
			...
			var prompt = {
				type: 'list',
				name: 'type',
				message: chalk.green('What type of decorator is this?'),
				choices: [
					'config',
					'common',
					'models',
		...
		promptForModuleLocation: function() {
			...
			var prompt = {
				type: 'input',
				name: 'location',
				message: chalk.green('Which folder should i create the decorator in?'),
				default: this.name
			};
	...
	writing: {
		...
		this.log(chalk.yellow('Dont forget to inject the provider which you want to configure'));
