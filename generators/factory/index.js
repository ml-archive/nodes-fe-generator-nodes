...
module.exports = nodesGenerator.extend({
	...
	constructor: function() {
		...
		this.option('skip-module', {
			desc: 'Skips registerring the factory as a standalone angular module',
			type: Boolean
		});
	...
	prompting: {
		...
		promptForModuleType: function() {
			...
			var prompt = {
				type: 'list',
				name: 'type',
				message: chalk.green('What type of factory is this?'),
				choices: [
					'common',
					'modules',
					'models'
		...
		promptForModuleLocation: function() {
			...
			var prompt = {
				type: 'input',
				name: 'location',
				message: chalk.green('Which folder should i create the factory in?'),
				default: this.name
			};
